...
| ۱۶ | `incident_detector.py` | تابع `detect_incident_candidate` با شدت medium/high، عنوان و دلیل فارسی، `is_duplicate`؛ برچسب فارسی برای ۷ دسته (شامل نرم‌افزار/سخت‌افزار). |
...
> این سرویس یک شیء **incident غنی‌تر** برمی‌گرداند (`severity`، `fa_reason_incident`، `matched_ticket_ids`، `avg_similarity_score`، `is_duplicate`). Backend دو فیلد §2.3 را از آن بیرون می‌کشد و بقیه را می‌تواند ذخیره کند. توجه: نام فیلد عنوان در خروجی ما `incident.fa_title_incident` است (نه `incident_title_fa`) — این تفاوت باید با تیم Backend هماهنگ باشد.
...
- `test_incident_detector.py` — ۴ تیکت → high، ۲–۳ → medium، ۱ → بدون رخداد، رشته‌های فارسی، `is_duplicate`.
